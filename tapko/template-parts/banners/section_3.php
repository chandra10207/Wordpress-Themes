...
// Get type
$block_type = get_theme_mod( 'my_setting_cho_3' );
$item_link = '';
$image_url = '';

if ( $block_type == 'product' ) {
	$product_id = get_theme_mod( 'my_setting_product_3' );
	if ( ! empty( $product_id ) ) {
	...
	$category_id = get_theme_mod( 'my_setting_category_3' );
	$category_img = get_theme_mod( 'image_category_image_3' );
	if ( ! empty( $category_id ) ) {
	    ...
	    if( ! empty( $category_img ) ){
	    	$image_url = $category_img;
		 	...
		 	$image_not = get_template_directory_uri() .'/img/default-banner-img/bannerimages3.jpg';
		}  
	}
} else {
   $ad_img = get_theme_mod( 'image_setting_ad_image_3');
   $item_link = get_theme_mod( 'my_setting_ad_url_3');
	   if(isset($ad_img)){
	   	$image_url = get_theme_mod( 'image_setting_ad_image_3');
	   }
}
