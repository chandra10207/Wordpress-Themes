...
// Get type
$block_type = get_theme_mod( 'my_setting_cho_2' );
$item_link = '';
$image_url = '';

if ( $block_type == 'product' ) {
	$product_id = get_theme_mod( 'my_setting_product_2' );
	if ( ! empty( $product_id ) ) {
	...
	$category_id = get_theme_mod( 'my_setting_category_2' );
	$category_img = get_theme_mod( 'image_category_image_2' );
	if ( ! empty( $category_id ) ) {
	    ...
	    if( ! empty( $category_img ) ){
	    	$image_url = $category_img;
		 	...
		 	$image_not = get_template_directory_uri() .'/img/default-banner-img/bannerimages2.jpg';
		} 
	}
} else {
   $ad_img = get_theme_mod( 'image_setting_ad_image_2');
   $item_link = get_theme_mod( 'my_setting_ad_url_2');
	   if(isset($ad_img)){
	   	$image_url = get_theme_mod( 'image_setting_ad_image_2');
	   }
}
